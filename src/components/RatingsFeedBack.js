...
const RatingsFeedBack = ({ ratingData, commentData }) => {
    ...
    let [firstFourComments, setFirstFourComments] = useState(commentData.slice(0,4));
  ...
  return (
    <>
      <div className="grid grid-cols-auto md:grid-cols-2 w-fit md:w-[56rem] gap-4">
        <div className="w-[27rem] h-[11rem] border border-black rounded-[32px] relative bg-[#FDFBF9]">
          <BalloonsIcon className="absolute left-[50%] bottom-2" />
        </div>
        <div className="w-[27rem] h-[11rem] flex justify-between items-center">
          <div className="w-[12rem] h-[11rem] border border-black rounded-[32px] flex flex-col justify-center items-center">
            <p className="text-[32px] font-[700]">{rating}/{maxRating}</p>
      ...
      <button
        className="h-[48px] w-[196px] border rounded-[6px] self-center mt-4"
